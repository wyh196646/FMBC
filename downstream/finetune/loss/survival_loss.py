...
def nll_loss(logits,label, alpha=0.4, eps=1e-7):
...
class NLLSurvLoss(object):
    ...
    def __call__(self, logits,label, alpha=None):
        if alpha is None:
            return nll_loss(logits,label, alpha=self.alpha)
        else:
            return nll_loss(logits,label, alpha=alpha)
