...
from model_lib.timm_v2 import __version__
from model_lib.timm_v2.models._pretrained import filter_pretrained_cfg
...
def load_model_config_from_hf(model_id: str):
    ...
    cached_file = download_from_hf(model_id, 'config.py.json')
    ...
    # NOTE currently discarding parent config.py as only arch name and pretrained_cfg used in timm right now
    pretrained_cfg = hf_config['pretrained_cfg']
    pretrained_cfg['hf_hub_id'] = model_id  # insert hf_hub id for pretrained weight load during model creation
    pretrained_cfg['source'] = 'hf-hub'

    # model should be created with base config.py num_classes if its exist
    if 'num_classes' in hf_config:
        pretrained_cfg['num_classes'] = hf_config['num_classes']

    # label meta-data in base config.py overrides saved pretrained_cfg on load
...
def save_config_for_hf(
    ...
    # set some values at root config.py level
    hf_config['architecture'] = pretrained_cfg.pop('architecture')
    ...
    if 'labels' in model_config:
        _logger.warning(
            "'labels' as a config.py field for is deprecated. Please use 'label_names' and 'label_descriptions'."
...
def save_for_hf(
    ...
    config_path = save_directory / 'config.py.json'
    save_config_for_hf(
        model,
        config_path,
...
def push_to_hf_hub(
    ...
    with TemporaryDirectory() as tmpdir:
        # Save model weights and config.py.
        save_for_hf(
            model,
            tmpdir,
            model_config=model_config,
