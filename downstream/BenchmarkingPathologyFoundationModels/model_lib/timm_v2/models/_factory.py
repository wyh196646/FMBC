...
from model_lib.timm_v2.layers import set_layer_config
...
def create_model(
    ...
    Args:
        ...
        scriptable: Set layer config.py so that model is jit scriptable (not working for all models yet).
        exportable: Set layer config.py so that model is traceable / ONNX exportable (not fully impl/obeyed yet).
        no_jit: Set layer config.py so that model doesn't utilize jit scripted layers (so far activations only).
    ...
    ```py
    >>> from model_lib.timm_v2 import create_model
