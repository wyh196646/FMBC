...
from model_lib.timm_v2.models._features import FeatureListNet, FeatureDictNet, FeatureHookNet, FeatureGetterNet
from model_lib.timm_v2.models._features_fx import FeatureGraphNet
from model_lib.timm_v2.models._helpers import load_state_dict
from model_lib.timm_v2.models._hub import has_hf_hub, download_cached_file, check_cached_file, load_state_dict_from_hf
from model_lib.timm_v2.models._manipulate import adapt_input_conv
from model_lib.timm_v2.models._pretrained import PretrainedCfg
from model_lib.timm_v2.models._prune import adapt_model_from_file
from model_lib.timm_v2.models._registry import get_pretrained_cfg
...
def load_custom_pretrained(
    ...
    if not pretrained_cfg:
        _logger.warning("Invalid pretrained config.py, cannot load weights.")
        return
...
def load_pretrained(
    ...
    Args:
        ...
        pretrained_cfg (Optional[Dict]): configuration for pretrained weights / target data_lib
    ...
    pretrained_cfg = pretrained_cfg or getattr(model, 'pretrained_cfg', None)
    if not pretrained_cfg:
        raise RuntimeError("Invalid pretrained config.py, cannot load weights. Use `pretrained=False` for random init.")
...
def resolve_pretrained_cfg(
    ...
    if not pretrained_cfg:
        _logger.warning(
            ...
            f" Please add a config.py to the model pretrained_cfg registry or pass explicitly.")
...
def build_model_with_cfg(
    ...
    This helper fn aids in the construction of a model including:
      ...
      * passing through optional model_cfg for models with config.py based arch spec
      * features_only model adaptation
      * pruning config.py / model adaptation

    Args:
        model_cls (nn.Module): model class
        variant (str): model variant name
        pretrained (bool): load pretrained weights
        pretrained_cfg (dict): model's pretrained weight/task config.py
        model_cfg (Optional[Dict]): model's architecture config.py
        feature_cfg (Optional[Dict]: feature extraction adapter config.py
    ...
    # resolve and update model pretrained config.py and model kwargs
