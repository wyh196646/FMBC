...
def _parse_split_info(split: str, info: Dict):
    ...
    if 'tar' in split or '..' in split:
        ...
        # ex: `data_lib-split-{0000..9999}.tar|100000` for 9999 shards, covering 100,000 samples
        split = split.split('|')
        num_samples = 0
        split_name = ''
        if len(split) > 1:
...
def log_and_continue(exn):
    ...
    # NOTE: try force an exit on errors that are clearly code / config.py and not transient
...
class ResampledShards2(IterableDataset):
    """An iterable data_lib yielding a list of urls."""
