...
class ReaderHfids(Reader):
    ...
    def _lazy_init(self):
        ...
        # Distributed:
        # The data_lib has a number of shards that is a factor of `dist_num_replicas` (i.e. if `ds.n_shards % dist_num_replicas == 0`),
        # so the shards are evenly assigned across the nodes.
        # If it's not the case for data_lib streaming, each node keeps 1 example out of `dist_num_replicas`, skipping the other examples.
