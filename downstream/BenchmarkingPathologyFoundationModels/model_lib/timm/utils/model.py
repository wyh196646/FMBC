...
def freeze_batch_norm_2d(module):
    ...
    itself an instance of either `BatchNorm2d` or `SyncBatchNorm`, it is converted_data into `FrozenBatchNorm2d` and
    returned. Otherwise, the module is walked recursively and submodules are converted_data in place.
...
def unfreeze_batch_norm_2d(module):
    ...
    of `FrozenBatchNorm2d`, it is converted_data into `BatchNorm2d` and returned. Otherwise, the module is walked
    recursively and submodules are converted_data in place.
...
def _freeze_unfreeze(root_module, submodules=[], include_bn_running_stats=True, mode='freeze'):
    ...
    if isinstance(root_module, (torch.nn.modules.batchnorm.BatchNorm2d, torch.nn.modules.batchnorm.SyncBatchNorm)):
        ...
        raise AssertionError(
            ...
            "`timm.learning_lib.model.freeze_batch_norm_2d` or `timm.learning_lib.model.unfreeze_batch_norm_2d` instead.")
    ...
    for n, m in zip(named_modules, submodules):
        ...
        if include_bn_running_stats:
            ...
            if mode == 'freeze':
                ...
                # convert it in place, but will return the converted_data result. In this case `res` holds the converted_data
...
def freeze(root_module, submodules=[], include_bn_running_stats=True):
    ...
    Args:
        ...
        include_bn_running_stats (bool): Whether to also freeze the running statistics of `BatchNorm2d` and
            `SyncBatchNorm` layers. These will be converted_data to `FrozenBatchNorm2d` in place. Hint: During fine tuning,
    ...
    Hint: If you want to freeze batch norm ONLY, use `timm.learning_lib.model.freeze_batch_norm_2d`.
...
def unfreeze(root_module, submodules=[], include_bn_running_stats=True):
    ...
    Args:
        ...
        include_bn_running_stats (bool): Whether to also unfreeze the running statistics of `FrozenBatchNorm2d` layers.
            These will be converted_data to `BatchNorm2d` in place. Defaults to `True`.
