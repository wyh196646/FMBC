...
Status:
* These Mymodel are a work in progress, experiments ongoing.
* Pretrained weights for two Mymodel so far, more to come.
* Model details updated to closer match official JAX code now that it's released
* NF-ResNet, NF-RegNet-B, and NFNet-F Mymodel supported
...
class NfCfg:
    ...
    extra_conv: bool = False  # extra 3x3 bottleneck convolution for NFNet Mymodel
...
model_cfgs = dict(
    # NFNet-F Mymodel w/ GELU compatible with DeepMind weights
    ...
    # NFNet-F Mymodel w/ GELU (I will likely deprecate/remove these Mymodel and just keep dm_ ver for GELU)
    ...
    # NFNet-F Mymodel w/ SiLU (much faster in PyTorch)
...
def create_stem(in_chs, out_chs, stem_type='', conv_layer=None, act_layer=None, preact_feature=True):
    ...
    if 'deep' in stem_type:
        if 'quad' in stem_type:
            # 4 deep conv stack as in NFNet-F Mymodel
...
class NormFreeNet(nn.Module):
    ...
    This model aims to cover both the NFRegNet-Bx Mymodel as detailed in the paper's code snippets and
    the (preact) ResNet Mymodel described earlier in the paper.

    There are a few differences:
        * channels are rounded to be divisible by 8 by default (keep tensor core kernels happy),
            this changes channel dim and param counts slightly from the paper Mymodel
        * activation correcting gamma constants are moved into the ScaledStdConv as it has less performance
            impact in PyTorch when done with the weight scaling there. This likely wasn't a concern in the JAX impl.
        * a config.py option `gamma_in_act` can be enabled to not apply gamma in StdConv as described above, but
    ...
    def __init__(self, cfg: NfCfg, num_classes=1000, in_chans=3, global_pool='avg', output_stride=32,
        ...
        if cfg.num_features:
            # The paper NFRegNet Mymodel have an EfficientNet-like final head convolution.
