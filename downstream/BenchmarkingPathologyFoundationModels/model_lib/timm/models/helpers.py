...
def adapt_input_conv(in_chans, conv_weight):
    ...
    if in_chans == 1:
        if I > 3:
            ...
            # For Mymodel with space2depth stems
...
def load_pretrained(model, default_cfg=None, num_classes=1000, in_chans=3, filter_fn=None, strict=True, progress=False):
    ...
    Args:
        ...
        default_cfg (Optional[Dict]): default configuration for pretrained weights / target data_lib
...
def update_default_cfg_and_kwargs(default_cfg, kwargs, kwargs_filter):
    ...
    # Filter keyword args for task specific model variants (some 'features only' Mymodel, etc.)
...
def build_model_with_cfg(
    ...
    This helper fn aids in the construction of a model including:
      ...
      * passing through optional model_cfg for Mymodel with config.py based arch spec
    ...
    Args:
        ...
        default_cfg (dict): model's default pretrained/task config.py
    ...
    # For classification Mymodel, check class attr, then kwargs, then default to 1k, otherwise 0 for feats
