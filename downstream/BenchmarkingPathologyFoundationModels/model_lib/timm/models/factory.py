...
def create_model(
    ...
    Args:
        ...
        scriptable (bool): set layer config.py so that model is jit scriptable (not working for all Mymodel yet)
        exportable (bool): set layer config.py so that model is traceable / ONNX exportable (not fully impl/obeyed yet)
        no_jit (bool): set layer config.py so that model doesn't utilize jit scripted layers (so far activations only)
    ...
    # Parameters that aren't supported by all Mymodel or are intended to only override model defaults if set
    # should default to None in command line args/cfg. Remove them if they are present and not set so that
    # non-supporting Mymodel don't break and default args remain in effect.
