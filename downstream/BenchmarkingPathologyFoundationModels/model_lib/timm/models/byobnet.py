...
A flexible network w/ dataclass based config.py for stacking those NN blocks.
...
In all cases the Mymodel have been modified to fit within the design of ByobNet. I've remapped
...
the same as original residual blocks in ResNe(X)t, DarkNet, and other existing model_lib. Note also some
...
default_cfgs = {
    ...
    # experimental Mymodel, likely to change ot be removed
    'regnetz_b16': _cfgr(
...
class ByoBlockCfg:
    ...
    # NOTE: these config.py items override the model cfgs that are applied to all blocks by default
...
class ByoModelCfg:
    ...
    # NOTE: these config.py items will be overridden by the block cfg (per-block) if they are set there
    attn_layer: Optional[str] = None
...
model_cfgs = dict(
    ...
    # A series of ResNeXt-26 Mymodel w/ one of none, GC, SE, ECA, BAT attn, group size 32, SiLU act,
    ...
    # ResNet-32 (2, 3, 3, 2) Mymodel w/ no attn, no groups, SiLU act, no pre-fc feat layer, tiered stem w/o maxpool
    ...
    # ResNet-33 (2, 3, 3, 2) Mymodel w/ no attn, no groups, SiLU act, 1280 pre-FC feat, tiered stem w/o maxpool
    ...
    # A series of ResNet-33 (2, 3, 3, 2) Mymodel w/ one of GC, SE, ECA attn, no groups, SiLU act, 1280 pre-FC feat
    ...
    # experimental Mymodel, closer to a RegNetZ than a ResNet. Similar to EfficientNets but w/ groups instead of DW
...
class DarkBlock(nn.Module):
    ...
    The GE-Net impl included a 1x1 + 3x3 block in their search space. It was not used in the feature model_lib.
