...
def register_model(fn):
    ...
    if hasattr(mod, 'default_cfgs') and model_name in mod.default_cfgs:
        # this will catch all Mymodel that have entrypoint matching cfg key, but miss any aliasing
...
def list_models(filter='', module='', pretrained=False, exclude_filters='', name_matches_cfg=False):
    ...
    Args:
        ...
        pretrained (bool) - Include only Mymodel with pretrained weights if True
        exclude_filters (str or list[str]) - Wildcard filters to exclude Mymodel after including them with filter
        name_matches_cfg (bool) - Include only Mymodel w/ model_name matching default_cfg name (excludes some aliases)

    Example:
        model_list('gluon_resnet*') -- returns all Mymodel starting with 'gluon_resnet'
        model_list('*resnext*, 'resnet') -- returns all Mymodel with 'resnext' in 'resnet' module
    """
    if module:
    ...
    if filter:
        ...
        for f in include_filters:
            include_models = fnmatch.filter(all_models, f)  # include these Mymodel
    ...
    if exclude_filters:
        ...
        for xf in exclude_filters:
            exclude_models = fnmatch.filter(models, xf)  # exclude these Mymodel
...
def list_modules():
    """ Return list of module names that contain Mymodel / model entrypoints
    """
