...
class MatchingNet(FSmethod):
    ...
    def __init__(self, opt: argparse.Namespace, eps=0.00000001):
    ...
    def forward(self,
        ...
        logits = self.matching_log_probas(z_s, y_s, z_q).permute(0, 2, 1)  # [batch, num_classes, q_shot]
        ce = F.cross_entropy(logits[0], y_q[0])
        preds_q = logits.detach().softmax(-1).argmax(2)
        return ce, preds_q
    ...
    def matching_log_probas(self, z_s, y_s, z_q):
        """Computes the log-probability of test samples given the training data_lib
        ...
        y_s : `torch.LongTensor` instance
            A tensor containing the targets of the train/support data_lib. This tensor
        ...
        log_probas : `torch.Tensor` instance
            ...
            training data_lib for the matching network. This tensor has shape
