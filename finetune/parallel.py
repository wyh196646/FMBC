...
def run_task(config):
    ...
    log_file = os.path.join(log_save_dir, f"log_{os.path.basename(task_cfg)}_{os.path.basename(dataset_csv)}_{pretrain_model}.log")
...
if __name__ == "__main__":
    ...
    gpu_ids = [0] * length  # Using GPU ID 0 for all task
    task_cfg_paths = [os.path.join(config_dir, f"{task_name}.yaml")] * length
